...
interface CreateTaskModalProps {
  ...
  onSubmit: (taskName: string, category: 'general' | 'live' | 'templates', subId: string | null) => void;
  onCancel: () => void;
}
...
export default function CreateTaskModal({ existingLiveFolders, existingTemplates, initialCategory, initialSubId, onSubmit, onCancel }: CreateTaskModalProps) {
  ...
  useEffect(() => {
    if (focusedIndex >= 0) {
      const el = document.getElementById(`target-task-item-${focusedIndex}`);
      if (el) {
          ...
          <div ref={targetDropdownRef} style={{ display: 'flex', flexDirection: 'column', gap: '4px', position: 'relative', zIndex: 10, flex: 1 }}>
            ...
            <div style={{ position: 'relative' }}>
              <input 
                ...
                onKeyDown={(e) => {
                  const isDown = e.key === 'ArrowDown' || (e.ctrlKey && e.key.toLowerCase() === 'j');
                  const isUp = e.key === 'ArrowUp' || (e.ctrlKey && e.key.toLowerCase() === 'k');
                  if (isDown || isUp) {
                    e.preventDefault();
                    e.nativeEvent.stopImmediatePropagation();
                    e.stopPropagation();
